  ...
  return (
    <span className="flex-1 grid grid-rows-[max-content_max-content]  gap-y-1">
      ...
      <div className="grid flex-1 h-1.5">
        <div></div>
        <div className="border-t border-solid border-grey-primary"></div>
      </div>
    </span>
  );
}
...
function Middle({ label }: { label: string }) {
  return (
    <span className="grid grid-cols-[max-content_max-content] grid-rows-[max-content_max-content] gap-y-1">
      <h2 className="col-start-2 text-grey-primary">{label}</h2>
      <div className="flex col-start-2 ">
        <div className="grid flex-1">
          <div></div>
          <div className="border-t border-solid border-grey-primary"></div>
        </div>
        <div className="rounded-full w-1.5 aspect-square bg-grey-primary mx-1.5"></div>
        <div className="grid flex-1">
          <div></div>
          <div className="border-t border-solid border-grey-primary"></div>
        </div>
    ...
    >
      ...
      <h2
        className={`col-start-2 ${
          selected ? "text-orange-primary" : "text-grey-primary"
        }`}
      >
        {label}
      </h2>
      <div className="flex col-start-2 ">
        ...
        <div
          className={`rounded-full w-1.5 aspect-square mx-1.5 ${
            selected ? "bg-orange-primary" : "bg-grey-primary"
          }`}
        ></div>
        <div className="grid flex-1">
          {isLeft ? (
